...
let defaultParamsToRegexify = ["api-key", "api_key", "apikey"];

//always replace capture group 2 (the kv pair value) in these patterns with 'redacted'
let defaultStringsAndPatternsToRedact = [
  new RegExp(/(Authorization\:\ Basic\ )([0-9a-zA-Z.=?&_-]+)/gi), //any basic auth headers
  new RegExp(/(apikey\=)([0-9a-zA-Z_-]+)/gi), //any api keys params in the body
  new RegExp(/(api\-key\=)([0-9a-zA-Z_-]+)/gi), //any api keys params in the body, dasherized
  new RegExp(/(api\_key\=)([0-9a-zA-Z_-]+)/gi) //any api keys params in the body, underscored
];
...
function regexifyFile(filepath, secrets_arr) {
  return new Promise(function(resolve, reject) {
    fs.readFile(filepath, "utf8", function read(err, data) {
      ...
      if (content[0].indexOf("REGEXP") !== -1) return resolve();
...
function redactFile(filepath, stringsAndPatternsToRedactArr) {
  stringsAndPatternsToRedactArr =
    stringsAndPatternsToRedactArr || defaultStringsAndPatternsToRedact;
  return new Promise(function(resolve, reject) {
    fs.readFile(filepath, "utf8", function read(err, data) {
      if (err) {
        throw err;
